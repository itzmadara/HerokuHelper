...
- VPS shift flow can copy saved bot entries to another saved VPS, install supported missing tools on the new VPS, and transfer saved `screen` workdirs/repositories when possible.
...
- Supported tool sync is best-effort and currently targets common bot tools such as `screen`, `docker`, `git`, `python3`, `pip3`, `node`, `npm`, `java`, and `pm2` when they exist on the old VPS but are missing on the new one.
